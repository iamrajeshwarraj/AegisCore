    ...
    run: async (client, message, args) => {
        ...
        if (input === 'add') {
                ...
                return message.channel.send({
                    embeds: [
                        new MessageEmbed()
                            .setColor(client.color)
                            .setDescription(
                                `${client.emoji.cross} | **Custom role** names cannot contain **mentions**. Please provide a different **name**.`
                            )
                    ]
                });
            }
            ...
            if (data) {
                ...
                return AegisCore(embed, client, message)
            }
        } else if (input == 'reset') {
            const data = await client.db?.get(`customrole_${message.guild.id}`)
            if (!data) {
...
async function AegisCore(embed, client, message) {
    let embeds = [],
        page = 0,
        i,
        k
